...
class TestPGVectorFiltering(unittest.TestCase):
    ...
    def test_is_qdrant_like_filter_detection(self):
        ...
        for case in qdrant_like_cases:
            with self.subTest(case=case):
                self.assertTrue(
                    is_qdrant_like_filter(case),
                    f"Should detect as qdrant-like: {case}"
                )
        ...
        for case in simple_cases:
            with self.subTest(case=case):
                self.assertFalse(
                    is_qdrant_like_filter(case),
                    f"Should NOT detect as qdrant-like: {case}"
                )
    ...
    def test_filter_clause_generation(self):
        ...
        test_cases = [
            ...
            # Multiple conditions
            (['payload->>%s = %s', 'payload->>%s = %s'], 
             "WHERE payload->>%s = %s AND payload->>%s = %s"),
    ...
    def test_comprehensive_filtering_scenarios(self):
        ...
        mock_filter = type('MockFilter', (), {
            'model_dump': lambda: {'must': [{'key': 'test'}]}
        })()
